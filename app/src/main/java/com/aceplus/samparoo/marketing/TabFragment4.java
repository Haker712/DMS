...
public class TabFragment4 extends Fragment {
    ...
    String outlet_stock_availability_id;
    int count = 0;
    Cursor cursor;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        activity.getSupportActionBar().setTitle("Outlet Stock Availability");
        activity.getSupportActionBar().setDisplayHomeAsUpEnabled(true);*/
        activity = (AppCompatActivity) getActivity();

        database = new Database(activity).getDataBase();

        cursor = database.rawQuery("select * from outlet_stock_availability", null);
        count = cursor.getCount();
        /*try {
            outlet_stock_availability_id = "OSA/" + MainFragmentActivity.userInfo.getString("userId") + "/" + MainFragmentActivity.customerId + "/" + (count + 1);
    ...
    private void insertintoDB() {
        ...
        for (SoldProduct soldProduct : soldProductList) {
            ...
            try{
                ...
                jsonObject.put("quantity",soldProduct.getQuantity());
            }catch(JSONException e){
                e.printStackTrace();
            }
        ...
        database.execSQL("INSERT INTO outlet_stock_availability VALUES (\""
